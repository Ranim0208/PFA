...
import { notifyReschedule, notifyPendingRegionalValidation } from "../services/notificationService.js";
import { notifyNewEvent, notifyPendingComponentValidation, notifyPendingIncubationValidation } from "../services/notificationService.js";
...
export const createCreathon = async (req, res) => {
  try {
    ...
    try {
      await notifyPendingComponentValidation(creathon);
    } catch (notifErr) {
      console.error("Error sending pending component notification:", notifErr);
    }
...
export const validateCreathonLogistics = async (req, res) => {
  try {
    ...
    try {
      await notifyPendingIncubationValidation(creathon);
      await notifyPendingRegionalValidation(creathon);
    } catch (notifErr) {
      console.error("Error sending pending notifications:", notifErr);
    }
...
export const validateCreathonLogisticsByGeneralCoordinator = async (
  ...
  try {
    ...
    console.log("✅ Créathon validé, envoi notification...");
    try {
  ...
  try {
    ...
    await sendEmail({
      ...
      html: `
          ...
          <p>L'équipe du Créathon "<strong>${updatedCreathon.title
          ...
          .map(
            (m) =>
              `<li>${m.firstName || "Inconnu"} ${m.lastName || ""} (${m.email || "Email non fourni"
              })</li>`,
          )
          .join("")}
          </ul>
          ...
          .map(
            (j) =>
              `<li>${j.firstName || "Inconnu"} ${j.lastName || ""} (${j.email || "Email non fourni"
              })</li>`,
          )
          .join("")}
          </ul>
